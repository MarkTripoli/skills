...
|---|---|---|---|---|
| Is the plan finished; which phase is next | `delivery-implement` `until_bash`, `next-phase`, `next-phase-auto` | `plan-remaining` | may only end the loop early | the `## Phase N` checkbox awk |
...
| Did the verification really pass | `delivery-verify` `verification-status` | `verification-status` | may only move a claim toward `failed` or `blocked` | the claimed status |
