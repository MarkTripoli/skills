...
Steering is owner-only. `setup --owner <U…>` names the Slack user whose thread replies become pending input; the daemon stamps that user on every run it opens, and replies from anyone else are ignored. A reply is never executed: the agent reads it, decides, and answers through `run resolve`. A direct message from anyone but the owner is answered once, `This assistant only takes instructions from its owner, <@owner>.`, and the sender is recorded in `refused_users`; every later DM from that sender is dropped silently.
