...
Primary grading checks returned image bytes, or separately inspected and hash-bound transformed payloads, rather than accepting a matching path alone. Required subject inspection must use bare image-path reads or bare video timestamp reads for image-payload identity; `?q=` image questions are text-only and count only as interpretation when paired with a bare binding read. Its pre-mutation receipt must persist `in-progress`, consumed round `1`, default limit `3`, the inspected finding, and an incomplete repair step. Resolve that step from the receipt's current structured state, not a required phrase; completed or contradictory state cannot establish a pending reservation. A final receipt with a different default limit fails. All counter-flow predicates use the same receipt-local charter resolution, including mapped IDs and equivalent Activate/Click actions. Inspection-only failure cases require failed Increment and passed Reset; viewer-blocked requires both untested. Unmapped IDs remain failures rather than being folded into coverage.

Required bare image-path reads and bare video timestamp reads are sequential evidence operations. Issue each one as a standalone tool call with no concurrent bash/write/read/browser/delegation/other tool call, wait for the returned image payload, then record the observation and trace/hash binding before issuing the next tool call.

OMP 18.1.22's video reader creates relative temporary frame and contact-sheet directories. The observer records its actual ffmpeg invocation, runtime call stack, active reads, exit, and output hash. Attribution requires the exact video read, process inputs/outputs, returned image bytes, temporary lifetime, and absence of an overlapping writer. A bare-video preview additionally binds every thumbnail to the sheet process that consumes it and to that sheet's successful read result. Unknown files, broken thumbnail chains, unmatched bytes, persistent files, and committed viewer output remain forbidden. This is not a filename-prefix exception or an operating-system sandbox; unsupported viewer shapes fail closed.

Before committing any terminal receipt, follow the companion's finalization checklist: retain all seven coverage columns, use observed timestamps or identify unavailable times, and reconcile current pending state with the actual result. Earlier reservations remain clearly historical. A failed/no-progress result does not permit a shortened coverage table or invented chronology. Preserve rejected receipts unchanged; prove instruction repairs with fresh subjects rather than editing old outcomes.
...
The denied-view case launches a fresh isolated HOME/config with one environment-only OAuth credential exported through `omp token <provider>`. It requires an explicit `--model` whose provider has an environment credential arrangement (`openai-codex` via `OPENAI_CODEX_OAUTH_TOKEN`, `anthropic` via `ANTHROPIC_OAUTH_TOKEN`). No credential is copied into evidence. The extension records the live effective settings because OMP 18.1.22's standalone `config list` does not apply the session overlay. A finite list permits text reads, unchanged fixed capture/finalization, and receipt-only Git commands; only the named receipt is writable. This fault boundary is not an operating-system sandbox.
...
Scenario definitions may declare `minMinutes` to enforce a floor on the time budget regardless of the `--max-time` CLI flag. The runner computes `max(--max-time, scenario.minMinutes)` so short CLI overrides do not undercut a scenario's proven minimum. `three-rounds` sets `minMinutes: 45` because baseline plus three productive passes — each requiring four recorded frames individually opened, a repair, and round reconciliation — cannot reliably finalize within 25 minutes.

The no-progress subject delegates one unused-setting edit to a real, bounded OMP worker. Review its retained trace and the unchanged defective handler, not a simulated worker result. The disclosed worker command is the delegation boundary: it verifies the receipt on disk before it starts anything and writes no reservation itself, so a missing, stale, or terminal checkpoint fails closed with the exact reason retained in `worker/delegation.jsonl` and the worker never runs. The zero-limit subject records truthful baseline evidence without editing source or checks. The four-counter case omits an explicit limit and requires baseline plus three productive passes: increment values `2222`, `1222`, `1122`, `1112`; every Reset stays `0`.
...
Open each case's recordings and exact retained subject image payloads before filling `review.json`. Required subject evidence-frame reads must be bare retained image paths or bare retained video timestamp selectors; `?q=` text answers do not establish returned-image identity unless a separate bare read binds the same sample. Large source PNG sheets may be resized to WebP by the subject's viewer; retain and independently inspect both identities instead of equating their hashes. Missing review, worker/session trace, transformed image, or any required pass observation must fail grading.

For these retained reviews, every required bare image-path read or bare video timestamp read must be a standalone sequential tool call. Do not bundle it with another `read`, `bash`, `write`, browser action, or any other tool. Wait for the returned image before recording the observation, trace/hash binding, or coverage verdict.
