...
Each invocation reserves its timestamped run directory with an exclusive filesystem create. Same-second invocations receive numeric suffixes and keep separate `.dist`, recordings, reports, and summaries. `latest` is replaced atomically through an invocation-owned temporary symlink, so concurrent runs never remove another run's link. `summary.json` is the run-completion authority: regrading requires a valid non-empty summary whose scenario entries match their retained `report.json` files. A complete run may be regraded with an intentional scenario subset, but an active run, corrupt completion metadata, or a selected scenario absent from the summary fails before phase grading.
...
Terminal scenarios opt in with `phaseType: "terminal"`. They declare exact `allowedChangedPaths`, retain before/after repository manifests, excluded-root manifests, local Git-config snapshots, semantic Git-index snapshots, and typed OMP exit status beside `answer.md`, require unchanged Git `HEAD` and task state, and reject next-skill handoff fences. The runner captures these states after fixture overlays and prompt preparation, immediately before OMP starts, then captures the after states immediately when OMP exits. It writes repository and excluded-root manifests before Git-config capture so an unsafe Git-root mutation remains retained evidence. Multiple terminal phases share one throwaway repository, so a scenario can prove a first run and byte-stable rerun. Live grading and regrading use the same retained exit status and snapshot comparisons. Regrading validates each manifest's discriminated record schema before scenario checks; unknown kinds, missing fields, extra fields, and malformed values fail as invalid evidence. A completed recording with missing or invalid exit or manifest evidence fails as incomplete evidence and remains in failure aggregation.

Terminal repository manifests exclude root `.git/`, `.agents/`, and `.omp/` harness entries; identically named nested directories remain graded repository paths and excluded roots never enter `allowedChangedPaths`. Validators reject absolute paths, empty or dot segments, parent traversal, root entries in the repository bucket, entries outside their excluded-root bucket, and retained file or link digests that disagree with their payload. Separate typed manifests compare all three root entries without following symlinks or opening FIFO and other special-entry payloads. Regular files and directories retain normalized permission bits, so chmod-only changes are graded. A regular-file read failure becomes bounded evidence with entry kind, normalized mode, operation, safe error class, and a null digest; payload bytes and raw error text are never retained. The `.git` manifest omits only `.git/config`, whose independent snapshot retains entry kind, permission mode, and, for a readable regular file, only its SHA-256 digest. Config bytes never enter retained evidence. An absent, symlinked, or non-directory `.git` yields a null config snapshot; its typed excluded-root record supplies the live and retained-regrade diagnostic without opening external bytes. An ordinary `.git/index` retains a type-only file record because read-only Git commands can refresh volatile stat-cache bytes. Its separate semantic manifest records each tracked path's stage, mode, object identity, and persistent assume-unchanged, skip-worktree, and representable intent-to-add flags. Git-index commands use explicit Git/work-tree paths and discard inherited repository, index, common-directory, and object routing overrides. A Git command or record-parser failure becomes bounded typed evidence containing an operation class, stable code, and regular-index digest, never raw stderr. That evidence detects valid-to-corrupt and corrupt-byte mutations while allowing identical pre-existing error evidence to compare unchanged. Snapshotting does not invoke Git through an absent, symlinked, or non-directory `.git` root. Index absence, creation, deletion, entry-kind changes, symlink targets, directory descendants, and special entries remain graded by the typed `.git` manifest. Every other Git-internal path remains exact-byte graded, including hooks and refs. A live OMP eval proves only local skill behavior exercised by its scenario. It does not prove provider observation or mutation, which remains outside the `/setup-repository` first-release evidence boundary.
...
npm run evals -- setup-repository-basic setup-repository-unresolved setup-repository-migration setup-repository-safety setup-repository-read-errors --keep
...
The fixture repository is the only codebase an artifact may describe. Saved source snapshots and local guides prevent reads from mutable host files. Regrading requires saved `.dist` templates, fixture sources, typed phase exit status, and schema-valid manifests; incomplete legacy recordings fail closed.
