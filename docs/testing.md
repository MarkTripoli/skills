...
When another agent harness supervises Atomic, keep that host and its owned device services alive across harness-session teardown; an open terminal screen alone does not prove liveness. After a service loss, restore and check the authorized targets before native resume, inspect run status and cached stages, and explicitly stop persistent services after acceptance finishes. Run `b5e7dca8-8820-4d41-a8a5-e342ba386c03` resumed from `crashed` after this restoration with its first four stages cached; that checkpoint reuse does not by itself prove the remaining delivery or UI assertions.
