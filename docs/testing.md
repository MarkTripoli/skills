...
Terminal scenarios opt in with `phaseType: "terminal"`. They declare exact `allowedChangedPaths`, retain before/after repository manifests and local Git-config snapshots beside `answer.md`, require unchanged Git `HEAD` and task state, and reject next-skill handoff fences. Multiple terminal phases share one throwaway repository, so a scenario can prove a first run and byte-stable rerun. Regrading uses the retained snapshots; older terminal recordings without the required snapshots are skipped instead of passed.

Terminal repository manifests exclude root `.git/`, `.agents/`, and `.omp/` harness directories; identically named nested directories remain graded repository paths. The runner checks root `.agents/` and `.omp/` separately and compares retained local Git-config snapshots without adding `.git/` to repository manifests. A live OMP eval proves only local skill behavior exercised by its scenario. It does not prove provider observation or mutation, which remains outside the `/setup-repository` first-release evidence boundary.
