...
Terminal scenarios opt in with `phaseType: "terminal"`. They declare exact `allowedChangedPaths`, retain before/after repository manifests, excluded-root manifests, local Git-config snapshots, and semantic Git-index snapshots beside `answer.md`, require unchanged Git `HEAD` and task state, and reject next-skill handoff fences. The runner captures these states after fixture overlays and prompt preparation, immediately before OMP starts, then captures the after states immediately when OMP exits. It writes repository and excluded-root manifests before Git-config capture so an unsafe Git-root mutation remains retained evidence. Multiple terminal phases share one throwaway repository, so a scenario can prove a first run and byte-stable rerun. Live grading and regrading use the same retained snapshot comparisons. A completed recording with a missing or invalid required manifest fails as incomplete evidence; it is never removed from failure aggregation.

Terminal repository manifests exclude root `.git/`, `.agents/`, and `.omp/` harness entries; identically named nested directories remain graded repository paths and excluded roots never enter `allowedChangedPaths`. Separate typed manifests compare all three root entries without following symlinks or opening FIFO and other special-entry payloads. The `.git` manifest omits only `.git/config`, which retains its independent typed snapshot and diagnostic while `.git` is a real non-symlink directory. An absent, symlinked, or non-directory `.git` yields a null config snapshot; its typed excluded-root record supplies the live and retained-regrade diagnostic without opening external bytes. An ordinary `.git/index` retains a type-only file record because read-only Git commands can refresh volatile stat-cache bytes. Its separate semantic manifest records each tracked path's stage, mode, object identity, and persistent assume-unchanged, skip-worktree, and representable intent-to-add flags. It excludes timestamps, device and inode numbers, sizes, working-tree bytes, and symlink payloads, so read-only `git status` and `git diff` remain no-ops while persistent index concealment fails live and retained grading. Index absence, creation, deletion, entry-kind changes, symlink targets, directory descendants, and special entries remain graded by the typed `.git` manifest. Every other Git-internal path remains exact-byte graded, including hooks and refs. A live OMP eval proves only local skill behavior exercised by its scenario. It does not prove provider observation or mutation, which remains outside the `/setup-repository` first-release evidence boundary.
