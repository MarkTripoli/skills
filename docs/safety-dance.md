...
Build from source with `cd tools/safety-dance && go build ./cmd/safety-dance`. Releases provide checksummed archives for Linux, macOS, and Windows; Windows archives contain `safety-dance.exe`. Verify an archive against `checksums.txt` before installation. Set `SD_HOME` to select the runtime home (default `~/.safety-dance`) and configure repository policy in `.safety-dance.yaml`.
