...
- Affected live reruns: current generic, exact-Name, Casey-to-Jordan, blocked, and installed standalone receipts pass with external `idb`, JEV, recorder, and `/tmp/jev-ios-text-helper-continuation.py -p`; earlier invalid-helper blocked evidence remains preserved. `xcrun simctl list devices` reports authorized `7A023F51-F0DA-4179-868B-19207E433651` as `Shutdown`; the owned socket is absent and unrelated processes were not touched.
