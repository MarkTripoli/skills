...
| A3 | Exact label-equal Name confirmation (`task.md:34`; claimed: yes). | Run with the documented helper unchanged and the original goal; retain raw helper I/O. | The wrapper-backed receipt is rejected because it rewrote the goal. The unchanged helper returned non-contract output/usage failure, so current exact-Name proof is unproven. | untested | hand | 2 |
| A4 | Genuine Casey-to-Jordan replacement without relaunch (`task.md:34`; claimed: yes). | Run with unchanged generic helper; preserve original seed and replacement prompts. | The wrapper-backed receipt is not accepted because it rewrote the Casey seed. Current unchanged-helper proof is pending/unproven. | untested | hand | 2 |
...
CR-001 remains open: unchanged helper proof for exact Name, replacement, and standalone is pending. No implementation approval is claimed here.
