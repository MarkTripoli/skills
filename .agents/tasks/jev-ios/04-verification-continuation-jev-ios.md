...
| P7 | Prove exact label-equal `Name` reaches `Confirmed Name`. | Wrapper-backed receipt is retained but rejected; unchanged-helper raw-goal attempt is retained as unproven. | pending |
| P8 | Prove Casey-to-Jordan replacement without relaunch. | Wrapper-backed receipt is retained but rejected because the seed goal was rewritten; unchanged-helper rerun remains pending. | pending |
...
| P12 | Verify installed standalone iOS execution with external drivers, credentials, helper, and recording integration. | Wrapper-backed standalone receipt is retained but rejected; unchanged documented-helper rerun remains pending. | pending |
