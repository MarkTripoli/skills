...
- [ ] `tmp=$(mktemp -d); trap 'rm -rf "$tmp"' EXIT; for runtime in claude-code codex oh-my-pi pi; do node scripts/build-runtimes.mjs --runtime "$runtime" --dest "$tmp/$runtime"; test -f "$tmp/$runtime/skills/safety-dance/SKILL.md"; test ! -e "$tmp/$runtime/agents/safety-dance.md"; test ! -e "$tmp/$runtime/agents/safety-dance.toml"; done`
