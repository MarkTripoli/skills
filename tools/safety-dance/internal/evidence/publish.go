...
const (
	...
	fetchRef = "refs/safety-dance/evidence-fetch"

	scratchIndexName = "safety-dance-evidence-index"
)
...
// assertEvidenceBranch refuses a branch that exists but is not a safety-dance
...
func assertEvidenceBranch(ctx context.Context, repoDir, tip, branch string) error {
	...
	if err != nil || string(content) != MarkerContent {
		return fmt.Errorf("refusing to publish evidence: branch %q already exists and is not a safety-dance evidence branch (invalid %s marker at its tip)", branch, MarkerPath)
	}
	return nil
}
...
// safety-dance author when the repository configures none, so publishing never
...
func commitTree(ctx context.Context, repoDir, tree, parent, message string) (string, error) {
	name := configuredIdentity(ctx, repoDir, "user.name", "safety-dance")
	email := configuredIdentity(ctx, repoDir, "user.email", "safety-dance@localhost")
	env := []string{
	...
	if strings.TrimSpace(message) == "" {
		message = "safety-dance: test evidence"
	}
