...
func TestPreReceiveHookScript(t *testing.T) {
	script := preReceiveHookScript("/opt/Safety Dance/safety-dance")
	for _, want := range []string{
		"#!/bin/sh",
		"SD_BIN='/opt/Safety Dance/safety-dance'",
...
func TestPostReceiveHookScript(t *testing.T) {
	script := postReceiveHookScript("/opt/Safety Dance/safety-dance")
	...
	if !strings.Contains(script, "SD_BIN='/opt/Safety Dance/safety-dance'") {
		t.Fatal("hook should embed the safety-dance executable path")
	}
	...
	if !strings.Contains(script, "\"$SD_BIN\" daemon notify-push") {
		t.Fatal("hook should execute the embedded binary path")
	}
	if !strings.Contains(script, "command -v safety-dance") {
	...
	if !strings.Contains(script, "safety-dance") {
		t.Fatal("hook should mention the command name")
	}
	if !strings.Contains(script, "____ance") {
		t.Fatal("hook should contain Safety Dance ASCII art banner")
	}
...
func TestShellSingleQuote(t *testing.T) {
	tests := []struct {
		...
		{"plain", "/usr/bin/safety-dance", "'/usr/bin/safety-dance'"},
		{"spaces", "/opt/Safety Dance/bin", "'/opt/Safety Dance/bin'"},
...
func TestPostReceiveHookScriptWithQuotedPath(t *testing.T) {
	script := postReceiveHookScript("/opt/it's here/safety-dance")
	if !strings.Contains(script, "SD_BIN='/opt/it'\"'\"'s here/safety-dance'") {
...
func TestPostReceiveHookScriptDoesNotEvaluatePushOptions(t *testing.T) {
	...
	fakeBin := filepath.Join(base, "fake-safety-dance")
...
func TestPostReceiveHook_ResolvesAbsoluteGateDir(t *testing.T) {
	...
	// Fake safety-dance binary: record the notify-push argv so we can inspect
...
func TestPostReceiveHook_SurfacesNotifyFailures(t *testing.T) {
	...
	// Fake safety-dance binary that always fails notify-push with a
