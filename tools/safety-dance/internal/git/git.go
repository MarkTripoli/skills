...
// SD_HOME cannot discover or mutate an ancestor worktree.
...
func FetchRemoteRef(ctx context.Context, dir, remote, remoteRef, expectedCommit string) error {
	temporaryRef := fmt.Sprintf("refs/safety-dance/fetch/%d-%d", os.Getpid(), time.Now().UnixNano())
	defer func() {
...
// it only for a local safety-dance gate trigger; delivery pushes keep the
// repository hook.
...
// local safety-dance gate trigger; delivery pushes keep the repository hook.
