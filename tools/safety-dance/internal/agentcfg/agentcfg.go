...
const (
	// MechanismUnsupported means safety-dance has no verified way to set the
	...
	// MechanismArgs means safety-dance injects native CLI flags into the argv
...
var harnesses = map[types.AgentName]harness{
	...
	// strictly. Neither has a mechanism safety-dance can set, so both knobs are
...
// explicit acp:<target> spellings. safety-dance never speaks ACP itself; it
...
func Validate(name types.AgentName, p Profile) error {
	...
	for _, entry := range []struct {
		...
		if entry.knob.mechanism == MechanismUnsupported {
			return fmt.Errorf("agent %q cannot express %s; safety-dance has no verified %s mechanism for it (%s)", name, entry.name, entry.name, escapeHatch(name))
		}
