...
//
// Why a sweep is needed at all. Every subprocess safety-dance spawns is already
...
// <SD_HOME>/worktrees/<repoID>/<runID> belongs to that run, no matter who its
...
//
// A process whose cwd is outside <SD_HOME>/worktrees and outside the run
...
type Options struct {
	// WorktreesRoot is <SD_HOME>/worktrees. Required.
...
// <SD_HOME>/worktrees that process stays reachable through the surviving root
