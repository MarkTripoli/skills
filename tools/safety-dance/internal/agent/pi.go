...
func (a *piAgent) runOnce(ctx context.Context, opts RunOpts) (*Result, error) {
	if opts.Session != nil && opts.Session.ID != "" && !isPiSessionID(opts.Session.ID) {
		// Pi accepts a path or partial UUID for --session. safety-dance persists
...
// populated SessionRef resumes the UUID that safety-dance previously recorded.
...
// Pi's CLI because safety-dance must never resume an ambiguous global session.
...
func buildPiPrompt(prompt string, schema json.RawMessage) string {
	...
	return prompt + "\n\n## safety-dance final output contract\n\n" +
...
func (p *piParser) handleAssistantEvent(raw any) {
	...
	switch evt["type"] {
	case "text_delta":
		// Emit just the incremental delta. safety-dance' OnChunk consumers
