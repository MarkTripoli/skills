...
// in Safety Dance's per-step user prompt.
const grokGateSystemPrompt = "You are a Safety Dance pipeline coding agent. Follow the user prompt. " +
...
// Safety Dance must not claim verified disable_project_settings support until a
...
func (a *grokAgent) runOnce(ctx context.Context, opts RunOpts) (*Result, error) {
	promptFile, err := os.CreateTemp("", "safety-dance-grok-prompt-*.txt")
	if err != nil {
