...
// live.
//
// By default a run worktree is created at <SD_HOME>/worktrees/<repoID>/<runID>,
...
// their settings by path ancestry, so a worktree under SD_HOME inherits none
...
// can only have run under <SD_HOME>/worktrees. Consulting the current
...
// but it never learns where SD_HOME is, and it is not the owner of what a run
// worktree does to the checkout it belongs to. Those two are this policy:
//
// A root inside SD_HOME collides with the daemon's own state. Inside
// <SD_HOME>/worktrees the collision is total: those entries are the ULID-named
...
// Elsewhere under SD_HOME it is just as destructive for being narrower -
// <SD_HOME>/logs makes a run's worktree its own log directory (paths.RunLogDir
// is <SD_HOME>/logs/<runID>), so removing the worktree at run end destroys the
// logs of the run that just finished. Nothing an operator can want lives under
// SD_HOME anyway: it carries none of the directory-scoped toolchain
...
func CheckPlacement(p *paths.Paths, checkout, root string, otherCheckouts ...string) error {
	...
	if Contains(home, root) {
		return fmt.Errorf("%q is inside safety-dance' own state directory (%q), where it would collide with the daemon's worktrees, logs, or gates; choose a directory outside it", root, home)
	}
...
// whose runs it would hold, or inside the directory safety-dance already owns.
//
...
// <SD_HOME>/logs and <nm_home>/Logs are ONE directory that no spelling
