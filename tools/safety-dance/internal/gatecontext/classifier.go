...
// are executing inside an active safety-dance validation step.
package gatecontext
...
func RefusalMessage(result Result) string {
	target := "an active safety-dance validation step"
	if result.RunID != "" && result.Phase != "" {
		target = fmt.Sprintf("safety-dance run %s, phase %s", result.RunID, result.Phase)
	} else if result.RunID != "" {
		target = "safety-dance run " + result.RunID
	}
