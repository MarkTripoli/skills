...
import (
	...
	gitpkg "github.com/MarkTripoli/skills/tools/safety-dance/internal/git"
...
func TestInit(t *testing.T) {
	...
	// Verify safety-dance remote was added to working repo.
	url, err := gitpkg.GetRemoteURL(ctx, workDir, "safety-dance")
	if err != nil {
...
func TestInitIsIdempotent(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, workDir, RemoteName); err != nil {
		t.Errorf("safety-dance remote missing after re-init: %v", err)
	} else if url != bareDir {
		t.Errorf("remote url = %q, want %q", url, bareDir)
	}
	dbRepo, err := d.GetRepoByPath(workDir)
...
func TestInitRefreshUsesPersistedRepoID(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("get safety-dance remote: %v", err)
	}
	if url != legacyBareDir {
		t.Errorf("safety-dance remote = %q, want %q", url, legacyBareDir)
	}
...
func TestInitRepairsBrokenGate(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, workDir, RemoteName); err != nil {
		t.Errorf("safety-dance remote not restored after repair re-init: %v", err)
	} else if url != bareDir {
...
// a working directory does not break init idempotency: the leftover safety-dance
...
func TestInitReattachesGateAfterWorkingDirRename(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, renamed, RemoteName); err != nil {
		t.Errorf("safety-dance remote missing after reattach: %v", err)
	} else if url != bareDir {
		t.Errorf("remote url = %q, want %q", url, bareDir)
	}
...
// new repo: it gets its own gate and the copied safety-dance remote is
...
func TestInitCreatesFreshGateForCopiedWorkingDir(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, copyDir, RemoteName); err != nil {
		t.Errorf("safety-dance remote missing on copy: %v", err)
	...
	if url, err := gitpkg.GetRemoteURL(ctx, workDir, RemoteName); err != nil {
		t.Errorf("original safety-dance remote missing: %v", err)
...
// safety-dance remote pointing into our repos dir with no matching DB record
...
func TestInitRepointsOrphanGateRemoteOnFreshInit(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, renamed, RemoteName); err != nil {
		t.Errorf("safety-dance remote missing: %v", err)
	} else if url != p.RepoDir(repo.ID) {
		t.Errorf("remote url = %q, want %q", url, p.RepoDir(repo.ID))
	}
}

func TestInitDoesNotOverwriteExistingSafetyDanceRemoteOnFreshInit(t *testing.T) {
	workDir := setupTestRepo(t)
	nmRoot := t.TempDir()
	p := paths.WithRoot(nmRoot)
	if err := p.EnsureDirs(); err != nil {
	...
	if err == nil {
		t.Fatal("expected init to fail when safety-dance remote already exists")
	}
	...
	if url != customRemote {
		t.Errorf("safety-dance remote = %q, want %q", url, customRemote)
	}
}
...
func TestEject(t *testing.T) {
	...
	_, err = gitpkg.GetRemoteURL(ctx, workDir, "safety-dance")
	if err == nil {
		t.Error("expected safety-dance remote to be removed")
	}
...
func TestInit_PostReceiveSurvivesHooksPathPoisoning(t *testing.T) {
	...
	if out, err := exec.Command("git", "-C", workDir, "push", "safety-dance", "HEAD:refs/heads/test-branch").CombinedOutput(); err != nil {
		t.Fatalf("push: %v: %s", err, out)
	}
