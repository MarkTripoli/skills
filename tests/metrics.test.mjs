...
function fixture() {
  ...
  add("r1", 9, "node_completed", "prompt", { type: "prompt", duration_ms: 1000, tokens: { input: 10, output: 20, cacheRead: 3, cacheWrite: 4 }, cost_usd: 0.25 }, "00:05:01");
...
function server() {
  ...
  const value = http.createServer((request, response) => {
    ...
    request.on("end", () => { requests.push({ method: request.method, path: request.url, headers: request.headers, body }); response.writeHead(200, { "content-type": "application/json" }); response.end('{"url":"/d/archon-delivery"}'); });
  });
...
test("collect and render all metric families", () => {
  ...
  db.close();
  assert.match(output, /archon_runs_total\{gates="pr",outcome="completed",workflow="delivery-full"\} 1/);
  assert.match(output, /archon_runs_total\{gates="all",outcome="failed",workflow="delivery-bugfix"\} 1/);
  assert.match(output, /archon_run_duration_seconds_bucket\{gates="pr",le="900",workflow="delivery-full"\} 1/);
  assert.match(output, /archon_run_duration_seconds_sum\{gates="pr",workflow="delivery-full"\} 600/);
  assert.match(output, /archon_run_duration_seconds_count\{gates="pr",workflow="delivery-full"\} 1/);
  assert.match(output, /archon_node_duration_seconds_sum\{node="bash",type="bash",workflow="delivery-bugfix"\} 2/);
  assert.match(output, /archon_node_failures_total\{node="bash",workflow="delivery-bugfix"\} 1/);
  assert.match(output, /archon_gate_wait_seconds_sum\{node="gate",workflow="delivery-full"\} 120/);
  assert.match(output, /archon_gate_decisions_total\{decision="rejected",node="gate-result",workflow="delivery-full"\} 1/);
...
test("push sends all configured targets and cursor filters Loki events", async (t) => {
  ...
  assert.ok(influx.body.includes("archon_runs_total"));
...
test("provision posts the dashboard and creates the Prometheus datasource", async (t) => {
  ...
  assert.equal(JSON.parse(dashboard.body).dashboard.uid, "archon-delivery");
  const datasource = target.requests.find((request) => request.path === "/api/datasources");
  assert.equal(JSON.parse(datasource.body).name, "archon-prometheus");
  await close(target.server);
});
