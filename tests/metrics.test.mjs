...
test("collect and render all metric families", () => {
  ...
  db.close();
  assert.match(output, /delivery_runs_total\{gates="pr",outcome="completed",workflow="delivery-full"\} 1/);
  assert.match(output, /delivery_runs_total\{gates="all",outcome="failed",workflow="delivery-bugfix"\} 1/);
  assert.match(output, /delivery_run_duration_seconds_bucket\{gates="pr",le="900",workflow="delivery-full"\} 1/);
  assert.match(output, /delivery_run_duration_seconds_sum\{gates="pr",workflow="delivery-full"\} 600/);
  assert.match(output, /delivery_run_duration_seconds_count\{gates="pr",workflow="delivery-full"\} 1/);
  assert.match(output, /delivery_node_duration_seconds_sum\{node="bash",type="bash",workflow="delivery-bugfix"\} 2/);
  assert.match(output, /delivery_node_failures_total\{node="bash",workflow="delivery-bugfix"\} 1/);
  ...
  assert.match(output, /delivery_tokens_total\{kind="cache_read",node="prompt",workflow="delivery-full"\} 3/);
  assert.match(output, /delivery_cost_usd_total\{node="prompt",workflow="delivery-full"\} 0.25/);
  assert.match(output, /delivery_run_duration_seconds_sum\{gates="all",workflow="delivery-lean"\} 900/, "a resumed run is measured from its first event to its last, not from the reset started_at");
