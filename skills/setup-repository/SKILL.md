...
3. Read [repository metadata](references/repository-metadata.md) completely. Before reading metadata bytes, call `lstat` on `<root>/ai-utilities.json`. Accept only absence or a regular, non-symlink file. A directory, valid or dangling symlink, FIFO, socket, block device, or character device is a path/type conflict: read no payload, write nothing, perform zero external operations, and report `Written: none` without exposing a target or payload. Observe the original bytes only after the regular-file gate passes, then observe local remote URLs from Git configuration without contacting those remotes. Parse the original bytes once and name that exact result `observedDocument`; do not initialize, overwrite, or substitute `observedDocument` with `null` after a successful parse.
4. Validate `observedDocument` before planning. First require that exact parsed value to be a top-level object; then require `onboarding`, when present, to be an object and validate its supported integer versions, current profile, provider-object shape, and every nested key against the secret-key rules in the metadata reference. Invalid JSON, an invalid owned-state type, an invalid version field, an unsupported owned schema or revision, or a secret-shaped field under `onboarding` produces a conflict receipt and zero writes. Report the JSON path, observed type or value class, and supported expectation without exposing a secret value or source excerpt.
5. Classify the validated `observedDocument` with the ordered migration table before considering any conflict receipt. Schema 1 at revision 1 with valid current fields is `Observed state: current`, has `Conflicts: none`, and plans the unchanged `observedDocument`; a conflict is available only when parsing or validation actually failed or no migration-table row supports the observed version pair. Compute the complete document and receipt plan in memory from `observedDocument`. Preserve each existing top-level value except `onboarding` without reconstructing, normalizing, sorting, or defaulting it. Assign only the planned replacement `onboarding` value.
...
6. Compare semantic documents before serialization. When no semantic change exists, retain the original bytes and permissions exactly and skip the write. When the plan changes semantics, serialize only as specified in the metadata reference, write a temporary sibling file, set that regular temporary file to the existing metadata file's permission bits, and atomically rename it to `ai-utilities.json`. On first creation, request owner-readable and owner-writable, non-world-writable permissions (`0600`); where POSIX modes apply, require the resulting file to remain owner-writable and not world-writable. Create the temporary file only after validation and planning succeed; remove it if permission setup or rename fails.
