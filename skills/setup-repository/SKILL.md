...
4. Parse and validate before planning. Invalid JSON, an invalid owned-state type, an unsupported owned schema, or a secret-shaped field under `onboarding` produces a conflict receipt and zero writes. Never expose a secret value in output.
5. Compute the complete document and receipt plan in memory. Preserve user-owned and unknown top-level values. In `reconcile`, initialize absent managed state or retain current valid managed state. In `reset-managed`, use the same safe local authority in this release: initialize absent state or retain current valid state; do not broaden authority over provider records or user-owned fields.
