...
8. **Finish when user is done**: When user says PRD is complete or solution fully fleshed out and approved, read `references/prd_final_answer.md`, save the file, follow template; fill `{artifact_link}` with a relative Markdown link to the saved file, `[NN-prd-slug.md](.agents/tasks/<slug>/NN-prd-slug.md)`, fill `{artifact_file}` with the saved file's name only (the template carries the `@`; name this artifact and no other file, never a path), and fill the `Check:` and `Known limits:` lines from the artifact's `### Verify` and `### Known limits` lists. Add no prose before or after the template. When not run by the workflow engine, commit the saved file with `git add <path>` as `docs(task): prd artifact`.
