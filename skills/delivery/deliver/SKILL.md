...
5. **Without Archon**: open the task directory and hand off to the chain's first skill.
   ...
   - Reply with `references/deliver_hand_answer.md`, every `<...>` slot filled: the pack, its confidence, the autonomy level and gates, the task directory, and the chain as the table in `workflows/delivery.md` lists it. Fill `{run_location}` and `<branch>` from observed git state per the conventions' placeholder rule (`{run_location}` is `` `<root>` on branch `<branch>` `` from `git rev-parse --show-toplevel` and `--abbrev-ref HEAD`, or `this checkout` outside a work tree), never invented; every phase runs from this checkout on this branch, because the committed task directory and the `@<file>` handoffs are relative to it. Outside a git work tree, choose the not-a-git-work-tree sentence. The gates that stay on are the replies the user reviews before pasting the next command; later phases are the skills the chain names.
