...
Wave 1 children and their start commands (run each from the project root; use shell single quotes and write each prompt apostrophe as `'\''`; each child runs as its own delivery run, cuts its worktree from the epic branch named by `--base`, and opens its pull request against that branch):
