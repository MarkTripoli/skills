...
6. **Final answer**: If unresolved design questions remain, follow `references/design_discussion_review_answer.md`. If all resolved, follow `references/design_discussion_final_answer.md`. Fill the chosen template exactly: `{artifact_link}` is a relative Markdown link to the saved file, `[NN-type-slug.md](.agents/tasks/<slug>/NN-type-slug.md)`, and `{artifact_file}` is the saved file's name only (the template carries the `@`; name this artifact and no other file, never a path). Do not add a separate summary or any prose before or after the template. When not run by the workflow engine, commit the saved file with `git add <path>` as `docs(task): design-discussion artifact`.
