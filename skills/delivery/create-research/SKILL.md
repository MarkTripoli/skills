...
2. **Read named files first**. Read references/research_template.md from this skill's directory. If the user or research-questions doc names files, read them completely before starting child workers. Read the current artifact of type `sources` when one exists; for a `web` question its excerpts are the external evidence, cited by the source's location and pointer, and a web worker runs only for what the sources leave open.
