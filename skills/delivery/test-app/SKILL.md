...
2. **Check for current app-test evidence.** When `review.browser` has a current iteration, this is a re-run after `iterate-implementation`: use it as input, re-run every step, and record a new iteration rather than editing it.
...
8. **Save the artifact.** Record the next immutable `review.browser` iteration through the conventions' Recording an artifact flow using the template. Preserve all required frontmatter, steps, findings, missing, and Human Review fields. Screenshots remain support files under `<task dir>/app-test/` and are referenced relatively. Commit the canonical artifact path, support screenshots, and `index.json` explicitly as `docs(task): app-test artifact`. A legacy task without `index.json` follows the conventions' legacy rules.
