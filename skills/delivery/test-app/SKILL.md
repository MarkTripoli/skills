...
Inputs: `kind` is `web`, `ios`, or `android`; `target` is optional and names what to launch (a URL, an iOS bundle id or `.app` path, an Android package or `.apk`). Under Archon both arrive in the node prompt; by hand they come from the request, and a missing `kind` is inferred from the repository (a `package.json` with a dev script is `web`, an `.xcodeproj` or `Package.swift` with an app target is `ios`, a `build.gradle` with an application plugin is `android`).
