...
Inside a task: `.agents/tasks/<slug>/evidence/<session-name>/` (one directory per recorded surface, plus `composite/` for a side-by-side), and a numbered receipt `NN-evidence-<slug>.md` in the task directory written from `references/evidence_template.md`, which is how later phases see that this phase ran. Before the first session, write `.agents/tasks/<slug>/evidence/.gitignore` containing the single line `*`: the task directory is committed history and the artifact commit runs `git add -A .agents/tasks`, so this file keeps recordings out of it. Outside a task: `.artifacts/evidence/<session-name>/`; append `.artifacts/` to `.gitignore` when the file exists and lacks it. Recordings are uploaded, never committed; the receipt is a task artifact like any other and, when not run by the workflow engine, is committed with `git add <path>` as `docs(task): evidence artifact`.
