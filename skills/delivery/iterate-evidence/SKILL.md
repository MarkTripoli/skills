...
Read [inspection and acceptance](references/inspection_acceptance.md) before the first inspection and use its claim-specific rules throughout. For each required flow, open that flow's specific extracted frame individually as a separate viewer call; a contact sheet or composite view showing multiple flows simultaneously does not substitute for per-flow individual inspection. For each recording session, also open the initial application state as a separately named frame — the initial-zero frame showing the starting value before any interaction — as a distinct viewer call; do not infer the starting state from a fresh page, a probe, or a text description. **Extract the initial-zero frame at the manifest's `initial` action's `videoTime` (read from `capture.json`'s `actions` array), not from a recorder-generated `test_start` frame or any frame at or after the first click action.** Use the read tool with an explicit timestamp selector (e.g., `read video_path:Ts` where T is the `initial` action's `videoTime` from `capture.json`) or equivalent direct ffmpeg extraction; name the file to include `initial` and cite the exact timestamp (e.g., `initial-state-1.125s.png`). Record exact samples, visible application state, expected outcome/source, tool/reviewer, and timing for each opened frame.

For all required flows — not just the initial-zero frame — frame observation binds through the returned image hash using either form: (a) a retained named PNG/JPEG frame extracted from the recording, or (b) the retained video file read with an explicit timestamp selector (e.g., `read video_path:Ts`). In form (a), record the file's SHA-256. In form (b), record the SHA-256 of the image returned by the read call. For non-initial flows, the timestamp must fall at or after the flow's action `videoTime` from `capture.json`; for the initial-zero frame, it must be strictly before the first click's `videoTime`. See [inspection and acceptance](references/inspection_acceptance.md) for full binding and timing requirements.
