...
Record live behavior. Look at recorded pixels. Fix supported defects. Verify new recordings inside fixed allowance. Agent own decisions. One immutable receipt series hold history. No plan, worker, controller, Atomic install need.
...
New indexed run: allocate `evidence.iteration` through collection Recording an artifact flow and draft [receipt template](references/evidence_iteration_template.md) at returned staging path. Continuation read current iteration, allocate successor, copy current content into staging, then append next state there. Every durable checkpoint record staged successor and allocate next one; never edit recorded iteration. Legacy task without `index.json` keep its numbered receipt and update that file under collection legacy rule. Freeze these inputs before record or repair:
...
Run `python3 "$EVIDENCE" doctor`. Record capture power and real pixel-viewing power separate. Recorder healthy not prove model can see media. Before first capture, use recorder ignored task evidence storage: `<task-root>/<slug>/evidence/.gitignore` hold `*`. Give each capture own session dir under it. Keep baseline, failed captures, raw footage, reports, manifests, review frames.
...
1. Check leftover allowance and persist coherent reservation before delegate repair or touch source/checks:
   ...
   **Reservation write checkpoint (hard gate):** After write all reservation fields — frontmatter `consumed_rounds`, matching round record (heading, pre-round IDs, reservation row), Delivery state — record successor iteration, then read canonical recorded file and confirm `consumed_rounds` show bumped value. Record confirm in newly allocated staged successor as "Reservation persisted before any edit: yes". Only after this on-disk confirm may you write any repairable source or check path, delegate repair to any worker, or fire any other mutating command. Edit to app.js, check.mjs, or any repairable path before this on-disk confirm = ordering violation. Catch it and stop blocked, not keep going. Legacy task use its same numbered receipt for this checkpoint.
...
For indexed task, append observations, state transitions, rounds, authorizations, stop decisions in staged successor and record it as next `evidence.iteration`; current summaries must not replace copied history. Each persisted boundary gets one new immutable iteration. Legacy task without `index.json` updates same numbered receipt under legacy rule. Keep app source commits separate from receipt commits. Commit delegated source changes before artifact commit. Commit canonical receipt path and `index.json` explicitly as `docs(task): evidence-iteration artifact`. Media stay ignored. No Git? Report it uncommitted. Publishing, when asked, follow recorder upload procedure and need reopen destination to confirm playback.
