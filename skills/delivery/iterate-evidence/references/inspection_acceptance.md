...
**Per-flow individual frame requirement:** Before marking any required flow as passed, failed, or untested in the coverage table, open that flow's specific frame by name as a distinct viewer call and record what its pixels show. A contact sheet or composite view covering multiple flows at once does not count as individual per-flow inspection; each coverage-table row must be backed by its own named opened frame and an observed pixel value. A flow whose frame was not individually opened may only be recorded as `untested` with the reason.

**Frame identity:** A frame observation binds either (a) a retained named PNG/JPEG frame extracted from the recording, or (b) the retained video file read with an explicit timestamp selector (e.g., `read video.webm:Ts` where T is the target time in seconds). Both forms are equivalent and must bind the observation through the returned image hash: in form (a) record the extracted file's SHA-256; in form (b) record the SHA-256 of the image OMP returned from the timestamp-selector read. For non-initial flows, the timestamp must fall at or after the flow's action `videoTime` from `capture.json`. For the initial-state frame, the timestamp must be strictly before the first click action's `videoTime`. A mismatched hash or an out-of-window timestamp fails the observation regardless of the form used.
