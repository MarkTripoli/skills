...
**Sequential viewer call requirement:** Every required bare image-path read or bare video-timestamp read must be issued as its own standalone tool call, with no concurrent `bash`, `write`, `read`, browser, delegation, or other tool call in the same tool window. Wait for the returned image payload before recording the observation, hash, trace binding, or coverage row, then issue the next tool call. Parallel inspection is not valid evidence because viewer temporary files must be attributable to one completed read lifetime.
