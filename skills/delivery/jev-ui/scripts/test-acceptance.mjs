...
test('evidence handoff keeps source and target identity',()=>{
  const args=evidenceStartArgs({surface:'android',target:AUTHORIZED.android,evidenceDir:isolated()});
  assert.deepEqual(args.slice(0,2),['start','--output']); assert.equal(args[3],'--title'); assert.equal(args[4],'JEV fixture confirmation'); assert.equal(args[5],'--source'); assert.equal(args[6],'android');
...
test('acceptance passes limits, target and recorder status without rewriting failures',async()=>{
  const calls=[];
  const result=await runAcceptance({config:{surface:'android',target:AUTHORIZED.android,evidenceDir:isolated(),maxActions:2,maxModels:3},captureScreenshot:async()=>{},recorder:{start:async args=>{calls.push(['start',args]);return {session:'/tmp/session'};},stop:async(...args)=>calls.push(['stop',args])},controller:async input=>{calls.push(['controller',input]);return {status:'failed',reason:'postcondition'};},session:{id:AUTHORIZED.android}});
...
test('passed controller cannot bypass incomplete recorder finalization',async()=>{
  const result=await runAcceptance({config:{surface:'android',target:AUTHORIZED.android,evidenceDir:isolated()},session:{id:AUTHORIZED.android},captureScreenshot:async()=>{},recorder:{start:async()=>({session:'/tmp/session'}),stop:async()=>({verified:false})},controller:async()=>({status:'passed',target:{id:AUTHORIZED.android},decisions:[{model:'jev-latest',usage:{}}],expectedPostconditions:['Confirmed'],observations:[{target:{id:AUTHORIZED.android},elements:[{role:'status',value:'Confirmed'}]}]})});
...
test('replacement mode launches the empty fixture once and runs two genuine stages',async()=>{
  ...
  const result=await runAcceptance({
    config:{surface:'android',target:AUTHORIZED.android,evidenceDir:isolated(),seedGoal:'Set Name to exactly Casey',seedExpected:'Confirmed Casey',goal:'Replace Casey with Jordan',expected:'Confirmed Jordan',maxActions:4,maxModels:4},
    captureScreenshot:async()=>{},recorder:{start:async()=>({session:'/tmp/session'}),annotate:async()=>{},stop:async()=>evidence},
