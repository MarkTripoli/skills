...
Native receipts retain the selected Android serial in `target.id`; editable elements retain `focused` and `editable` metadata, and each actionable element retains observed bounds. Missing or inconsistent identity, app scope, bounds, or supported text is non-green and does not invoke the driver.
