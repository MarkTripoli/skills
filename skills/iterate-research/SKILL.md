...
6. **Final answer**. Read references/research_final_answer.md and respond using that template only. Fill `{artifact_link}` with a relative Markdown link to the saved file, `[NN-type-slug.md](.agents/tasks/<slug>/NN-type-slug.md)`. The last lines must be exactly one fenced `text` block copied from the template. Never repeat the current command `/iterate-research` as the next step. Choose the variant matching `workflow` in `task.md`: `full` -> `/create-design-discussion`, `lean` -> `/create-structure-outline`, `prd` -> `/create-prd`; delete the instruction line and the other variants.
