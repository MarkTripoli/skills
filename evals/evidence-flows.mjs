...
// follows a comma or a spaced slash in the retained receipts.
...
function actionFlow(action) {
  return flowName(action.replace(/^activate\s+/i, "").replace(/\s+(?:exactly\s+)?once\b/i, ""));
}
...
export function counterFlowCoverage(text) {
  ...
  for (const cells of rows(section(text, "### Targets and regression charter") ?? "")) {
    ...
    if (declared.semantic && action && declared.semantic !== action) conflict = true;
    if (!declared.id || !action) continue;
  ...
  return {
    increment: !conflict && outcomes.increment.length > 0 && outcomes.increment.every((result) => result === "failed"),
    reset: !conflict && outcomes.reset.length > 0 && outcomes.reset.every((result) => result === "passed"),
  };
}
