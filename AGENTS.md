...
- `.archon/workflows/delivery/` holds the Archon blocks and packs that chain the phases, each with `fixtures/` dry-run cases; `start/` is the one-command entry that routes a request, `program/` the PRD-to-children chain, `wave/` and `epic-wave/` launch epic children as unattended runs; every `prompt:` node names a model tier (`docs/model-routing.md`) and the `app-test` block runs when a pack gets `--input app_test=web|ios|android` (`docs/app-testing.md`). `delivery-omp/` is generated from it by `scripts/build-packs.mjs` and never edited.
